...
export const fetchTopItems = async (days: number = 1): Promise<TopItem[]> => {
  const response = await api.get('/api/admin/analytics/items', { 
    params: { days } 
  });
...
export const fetchOrderQueue = async (): Promise<Order[]> => {
  const response = await api.get('/api/admin/orders/queue');
...
export const bulkUpdateOrders = async (orderIds: string[], status: string, estimated_pickup_time?: string) => {
  const response = await api.patch('/api/admin/orders/bulk-update', {
    orderIds,
    status,
...
export const updateOrderPriority = async (orderId: string, priority: string) => {
  const response = await api.patch(`/api/admin/orders/${orderId}/priority`, { priority });
  return handleApiResponse(response);
};

export const assignOrderToStaff = async (orderId: string, staffId: string) => {
  const response = await api.patch(`/api/admin/orders/${orderId}/assign`, { staff_id: staffId });
  return handleApiResponse(response);
};
